...
def _legacy_outbound_to_uri(ob):
    ...
    if uri.startswith("tamizdat://"):
        return uri
    if uri.startswith("samizdat://"):
        return "tamizdat://" + uri[len("samizdat://"):]
...
def _migrate_legacy_if_needed(con):
    ...
    for ob in legacy.get("outbounds", []):
        ...
        if kind == "direct" or tag == "direct":
            continue
        if kind not in ("tamizdat", "samizdat"):
            continue
...
def _migrate_legacy_inbounds_json_to_flat(con):
    ...
    Why so cautious: existing pre-Phase-2 installs on llm2 + odikee have
...
#
# Tamizdat is the wire-renamed fork of samizdat (URI scheme tamizdat://, HKDF
...
def make_tamizdat_uri(cfg, outbound=None, label_override=None):
    ...
    differentiation for routing is via auth_user route rules (same as samizdat).
...
# Why vendored: panel runs on RU VPS where CDN reachability is flaky.
# DO NOT switch to a CDN — operator decision documented in commit log.
        ...
        <div class="set-content">
          ...
          <section class="group" id="g-wgturn">
            ...
            <div class="fields">
              ...
              <div class="field">
                <div class="field-lbl"><div class="lbl">Fixed fallback outbound</div><div class="hint">Optional tag such as <code>sync2-justhost-fi</code>. Empty = normal routing/default outbound.</div></div>
                <div class="field-ctrl"><input type="text" id="wgTurnOutboundTag" class="mono" placeholder="sync2-justhost-fi"></div>
              </div>
...
// Pill for an outbound TAG (e.g. "odikee-fi") in routing-rule rows: always
...
function renderUsers(){
  ...
  for(const u of users){
    ...
    if(local){
      ...
      h+=`<td><div class="cell-meta" style="font-weight:700;color:var(--text)">${esc(u.local_tun_name||'taml0')} via ${esc(u.outbound_tag||'not selected')}</div><div class="cell-meta">LAN ${esc(u.local_iface||'not selected')}</div><div class="cell-meta">TCP + UDP; ${esc(quicMode)}</div></td>`;
    }else{
...
const _GEO_PRESETS = {
  ...
  geosite: [
    ...
    {name: 'category-ai-!cn',   desc: 'AI-сервисы кроме CN (Anthropic, OpenAI, Gemini, Perplexity, Mistral)'},
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/", ""):
            ...
            # "ens18 — 185.193.102.213, 38.135.53.241" labels. Tries iproute2
