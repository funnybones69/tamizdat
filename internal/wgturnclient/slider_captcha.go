...
type captchaBootstrap struct {
	...
	// VK rotates this fallback in the captcha JavaScript bundle. A generated
	// or stale value is a bot signal and makes the checkbox return BOT.
	DebugInfo string
}
...
func fetchCaptchaDebugInfo(ctx context.Context, html string, client tlsclient.HttpClient, profile BotProfile) string {
	m := reCaptchaJSURL.FindStringSubmatch(html)
	if len(m) < 2 {
		return ""
	}
	req, err := fhttp.NewRequestWithContext(ctx, "GET", m[1], nil)
	if err != nil {
		return ""
	}
...
func buildCaptchaDeviceJSON(profile BotProfile) string {
	// The live widget reads exactly these 14 keys. Historical BotProfile JSON
	// also contained timezone/platform/vendor fields; those extras now trigger
	// BOT. Match the current browser object shape instead.
